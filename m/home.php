...
if ($act == 'index') {
	...
	$manager_image->set('type', 'banner');
	$manager_image->set('is_hidden', '0');
	$st->assign('lBanner', $manager_image->filter(''));

	$products = new products();
	$st->assign('lProducts', $products->list_product(0, 8));

	$category = new category();
	$st->assign('lCategory',$category->list_category('sex'));
	$wishlists = new wishlists();
	if(isset($_SESSION['user'])){
	$wishlist = $wishlists->getWishlistByUser($_SESSION['user']['id']);	
	} else {
		$wishlist = [];
	}
	$st->assign('lWishlists', $wishlist);
